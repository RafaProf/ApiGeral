...
namespace ApiGeral.Netdb
{
    public class ConnEMT
    {
        ...
        static readonly string databaseName = "emt_ip_um";
...
"group by usuario", connectionEmt);
        ...
        public static bool GetProducaoGeralIP(string date1, string date2)
        {
            Global.LimparListas();
            connectionEmt.Open();

            NpgsqlCommand cmd = new NpgsqlCommand(@"select usuario , COUNT(*) QTDE from (
select distinct usuario, mslink from historico_ip hi where date(data_operacao) between '" + date1 + "' and '" + date2 + "' and " +
"tipo_operacao = 'INCLUSAO') TOT " +
"group by usuario", connectionEmt);
    ...
    " group by data_operacao, usuario order by data_operacao asc;", connectionEmt);
        ...
        public static bool GetProducaoIndivIp(string data, string usuario)
        {
            Global.LimparListas();
            connectionEmt.Open();

            NpgsqlCommand cmd = new NpgsqlCommand(@"select usuario, data_operacao, COUNT(*) QTDE from (
	select distinct usuario, mslink, date(data_operacao) as data_operacao 
	from historico_ip hi 
	where date(data_operacao) between '" + data + "' and current_date " +
    " and tipo_operacao = 'INCLUSAO') TOT where usuario like '%" + usuario + "%' " +
    " group by data_operacao, usuario order by data_operacao asc;", connectionEmt);
...
"' group by c.id_coleta, c.data_criacao, c.us_criacao, c.status, l.lc_descricao ", connectionEmt);
